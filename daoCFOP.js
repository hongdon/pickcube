...
var daoCFOP = module.exports={
		...
		findf2l : function(data,res){
			...
			f2ldic.find({$elemMatch :{cubeobj: data.cubeObj}},function(err,results){
				console.log(results);
			})
			
			f2ldic.find({$elemMatch :{cubeobj: data.cubeObj}}).count(function(err,results){
				//console.log('asfsafsdfsadfsdgsadg');
				console.log(results);
				totalpage = results;
			})
			f2ldic.find({$elemMatch :{cubeobj: data.cubeObj}}).limit(5).skip(data.page * 5).sort({view : -1}).exec(function(err, results) {
				if(err){
					console.log(err);
				}
